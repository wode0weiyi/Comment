...
public class SwiftProgressHUD {
    ...
    ///等待加载，下方有文字
    @discardableResult
    public class func showWait(text:String)->UIWindow?{
        if let _ = UIApplication.shared.keyWindow {
            return SwiftProgress.waitWithText(title: text)
        }
        return nil
    }
...
//        }
    ...
    @discardableResult
    static func waitWithText(title:String, backgroundColor: UIColor = yj_showHUDBackColor, scale: Double = 1.0)->UIWindow{
        let frame = CGRect(x: 0, y: 0, width: 88, height: 88)
        let window = UIWindow()
        window.backgroundColor = hudBackgroundColor
        window.rootViewController = UIViewController()
        let mainView = UIView()
        ...
        let ai = UIActivityIndicatorView(activityIndicatorStyle: UIActivityIndicatorViewStyle.whiteLarge)
        ai.frame = CGRect(x: 26, y: 10, width: 36, height: 36)
        ai.startAnimating()
        mainView.addSubview(ai)
        
        let lab = UILabel()
        lab.frame = CGRect(x: 0, y: 50, width: frame.size.width, height: 36)
